...
**Question #2**: Create a scatterplot to predict net tuition from the average faculty salary. Facet by the variable "Private". Use the [ggplot2 cheatsheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/data-visualization.pdf) to help you identify the necessary functions and arguments.

```{r, echo = FALSE, eval = FALSE}

ggplot(data = colleges, mapping = aes(x = Avg_Fac_Salary, y = Net_Tuition)) +
  geom_point() + 
  facet_wrap(~Private)
...
**Question #3**: Use the `filter()` function from the dyplr package to create a data frame containing only colleges located in the state of Iowa. Using these data, create a side-by-side box plot of admissions rates by public and private schools. Overlay the boxplot with points representing each school. Add a text annotation beside the outlier saying "Grinnell".

```{r, eval = FALSE, echo = FALSE}
require(dplyr)

iowa_colleges = filter(colleges, State == "IA")
ggplot(data = iowa_colleges, aes(x = Private, y = Adm_Rate)) + 
  geom_boxplot +
  geom_point() +
  annotate(geom = "text", x = 1.2, y = 0.25, label = "Grinnell")
```
...
**Question #4**: Modify the code below to make the shape squares with blue outlines and red fill. Then use `alpha =.4` to reduce the transparency of the squares.
...
ggplot(data = colleges[1:30,], mapping = aes(x = Net_Tuition, y = Private)) +
  geom_point() + 
  stat_summary(fun = mean, geom = "point", color = "cyan", size = 3) +
  stat_summary(fun.data = mean_se, geom = "errorbar", color = "cyan")
```
...
**Question #5**: Using the data subset containing only colleges located in Iowa (created earlier), create a graph similar to the example above but using the arguments `geom = "linerange"`, `alpha = 0.3`, `size = 4`, and `color = "darkslateblue"` to depict 1 standard error above/below the means of private and public colleges.

```{r, eval = FALSE, echo = FALSE}
ggplot(data = iowa_colleges,  mapping = aes(x = Net_Tuition, y = Private)) +
  geom_point() + 
  stat_summary(fun.data = mean_se, geom = "linerange", color = "darkslateblue", alpha = 0.3, size = 4)
```
...
**Question #6**:
Read in the Ames housing data from https://grinnell-statistics.github.io/Grinnell_R_Tutorials/data/AmesHousing.csv . Create a graphic that shows the relationship between the outcome variable "SalePrice" and the explanatory variables: "YearBuilt". Color by the number of rooms above ground and facet by quality of the kitchen. Add a simple linear regression line. Use the black white theme. Give the graph a title and clear labels.


```{r eval = FALSE, echo = FALSE}
mydf <- read.csv("https://grinnell-statistics.github.io/Grinnell_R_Tutorials/data/AmesHousing.csv")
head(mydf)

ggplot(data = mydf, aes(x = YearBuilt, y = SalePrice)) + 
  geom_point()  + 
  geom_smooth() +
  aes(colour = TotRmsAbvGrd) + 
  facet_wrap(~KitchenQual, ncol = 4) + 
  theme_bw()
...
Using the Ames housing data, remove the houses with poor kitchen quality, order the four remaining kitchen quality from worst to best, and convert number of rooms above ground to a categorical variable. Using this modified data, recreate the graph in Question 5.
