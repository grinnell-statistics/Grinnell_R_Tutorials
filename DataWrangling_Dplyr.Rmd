...
* You can use both `|` and `,` to specify multiple tests. For example, we can return all colleges with fewer than 2,000 undergraduate students in Wisconsin, Minnesota, or Iowa. 
...
```{r 4b, exercise = TRUE}
Cost1 <- filter(Colleges, !is.na(cost)) # This removes all rows that have NA (missing values) in the cost column 
...
* Find a complete list of tutorials at [Stat2Labs](https://stat2labs.sites.grinnell.edu/RTutorials.html).
