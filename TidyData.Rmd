...
This data set is not tidy because each row contains two cases: the city in 2003 and the city in 2009. Additionally, one of our variables of interest, **year**, is given in the column names `2003` and `2009`. Instead, **year** should be provided in just one column. In order to tidy these data, we need to 
...
#### Removing Text
Finally, we will modify the year column by removing the word rice from each cell. To do this, we can use the `parse_number` function from the `readr` package. This function drops any non-numeric characters before or after the first number.
